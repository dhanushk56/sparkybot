...
export async function updateReactionRolePanel(userId, guildId, key, data) {
  return call(`/api/guilds/${guildId}/reaction-roles/${encodeURIComponent(key)}`, {
    method: "PATCH",
    body: JSON.stringify({ ...data, user_id: userId }),
  });
}

export async function deleteReactionRolePanel(userId, guildId, key) {
  return call(`/api/guilds/${guildId}/reaction-roles/${encodeURIComponent(key)}?user_id=${userId}`, {
    method: "DELETE",
  });
}

export async function getAiModerationSuggestion(userId, guildId, situation) {
  return call(`/api/guilds/${guildId}/ai-suggest`, {
    method: "POST",
