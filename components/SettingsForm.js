    ...
    const renderWelcome = () => (
      <>
        ...
        <div className="field-group">
          ...
          <textarea className="field-input" rows="2" value={form.welcome_dm || ""} placeholder="Welcome to {servername}, {user}!" onChange={(e) => set("welcome_dm")(e.target.value || null)} />
        </div>
        <div className="field-group">
          <label>Auto-Roles</label>
          <RoleMultiSelect value={form.autoroles || []} onChange={set("autoroles")} roleOptions={roleOptions} placeholder="Select auto-roles..." />
        </div>
        <div className="field-group">
          <label>Welcome Embed Title</label>
          <input className="field-input" value={form.welcome_embed_title || ""} placeholder="Welcome to {servername}!" onChange={(e) => set("welcome_embed_title")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Welcome Embed Description</label>
          <textarea className="field-input" rows="3" value={form.welcome_embed_description || ""} placeholder="👋 Welcome {user}..." onChange={(e) => set("welcome_embed_description")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Welcome Embed Footer</label>
          <input className="field-input" value={form.welcome_embed_footer || ""} placeholder="ID: {id} • {servername}" onChange={(e) => set("welcome_embed_footer")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Welcome Outside Text</label>
          <input className="field-input" value={form.welcome_outside_content || ""} placeholder="{user}" onChange={(e) => set("welcome_outside_content")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Goodbye Embed Description</label>
          <textarea className="field-input" rows="2" value={form.goodbye_embed_description || ""} placeholder="👋 Goodbye {username}!" onChange={(e) => set("goodbye_embed_description")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Goodbye Embed Footer</label>
          <input className="field-input" value={form.goodbye_embed_footer || ""} placeholder="ID: {id}" onChange={(e) => set("goodbye_embed_footer")(e.target.value)} />
        </div>
        <div className="field-group">
          <label>Goodbye Outside Text</label>
          <input className="field-input" value={form.goodbye_outside_content || ""} placeholder="" onChange={(e) => set("goodbye_outside_content")(e.target.value)} />
        </div>
      </>
    );
    ...
    const renderAutoMod = () => (
      <>
        ...
        <div className="field-group">
          ...
          <input className="field-input" type="number" min="0" value={automod.mention_limit || 5} onChange={(e) => setNested("automod", "mention_limit")(parseInt(e.target.value) || 0)} />
        </div>
        <div className="field-group">
          <label>Bad Words (comma separated)</label>
          <input className="field-input" value={(automod.bad_words || []).join(", ")} placeholder="word1, word2" onChange={(e) => setNested("automod", "bad_words")(e.target.value.split(",").map(s => s.trim()).filter(Boolean))} />
        </div>
        <div className="field-group">
        ...
        <div className="field-group">
          ...
          <select className="field-input" multiple value={(automod.ignored_channels || []).map(String)} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, opt => Number(opt.value)); setNested("automod", "ignored_channels")(selected); }} style={{ height: "auto", minHeight: "60px" }}>
            {channelOptions.map((c) => <option key={c.id} value={c.id}>#{c.name}</option>)}
          </select>
        </div>
        <div className="field-group">
          <label>Ignored Roles</label>
          <RoleMultiSelect value={automod.ignored_roles || []} onChange={setNested("automod", "ignored_roles")} roleOptions={roleOptions} placeholder="Select ignored roles..." />
        </div>
      </>
    );
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".5rem" }}>
            {["message_delete","message_edit","member_join","member_leave","member_ban","member_unban","role_create","role_delete","role_update","channel_create","channel_delete","channel_update","voice_join","voice_leave","voice_move","nickname_change","member_role_update","invite_create","invite_delete"].map((ev) => (
              <div key={ev} className="toggle-row" style={{ padding: ".25rem 0" }}>
                <div>
                  <div className="toggle-label" style={{ fontSize: ".85rem" }}>{ev.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase())}</div>
                </div>
    ...
    const renderVerification = () => (
      <>
        ...
        <div className="field-group">
          ...
          <select className="field-input" value={form.verification_role_id || ""} onChange={(e) => set("verification_role_id")(e.target.value || null)}>
            <option value="">None</option>
        ...
        <div className="field-group">
          ...
          <input className="field-input" value={form.verification_category || "Verification"} placeholder="Verification" onChange={(e) => set("verification_category")(e.target.value)} />
        </div>
      </>
    );
    ...
    const renderTickets = () => (
      <>
        ...
        <div className="field-group">
          ...
          <input className="field-input" value={form.ticket_category || "Tickets"} placeholder="Tickets" onChange={(e) => set("ticket_category")(e.target.value)} />
        </div>
        <div className="field-group">
        ...
        <div className="field-group">
          ...
          <RoleMultiSelect value={form.ticket_staff_roles || []} onChange={set("ticket_staff_roles")} roleOptions={roleOptions} placeholder="Select staff roles..." />
        </div>
        <div className="field-group">
          ...
          <div className="toggle-row">
            ...
            <Toggle checked={!!ticketCustomization.enable_ratings} onChange={(v) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, enable_ratings: v } }))} />
          </div>
          <div className="toggle-row">
            <div>
            ...
            <Toggle checked={!!ticketCustomization.enable_history} onChange={(v) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, enable_history: v } }))} />
          </div>
          <div className="toggle-row">
            <div>
            ...
            <Toggle checked={!!ticketCustomization.enable_auto_close} onChange={(v) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, enable_auto_close: v } }))} />
          </div>
          <div className="field-group">
            <label>Default Auto-Close Hours</label>
            <input className="field-input" type="number" min="1" value={ticketCustomization.default_auto_close_hours || 24} onChange={(e) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, default_auto_close_hours: parseInt(e.target.value) || 24 } }))} />
          </div>
          <div className="field-group">
            <label>Reviews Channel</label>
            <ChannelSelect value={ticketCustomization.reviews_channel} onChange={(v) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, reviews_channel: v } }))} />
          </div>
          <div className="toggle-row">
            <div>
            ...
            <Toggle checked={!!ticketCustomization.show_available_categories} onChange={(v) => setForm(f => ({ ...f, ticket_customization: { ...f.ticket_customization, show_available_categories: v } }))} />
          </div>
        </div>
      </>
    );
    ...
    const renderMusic = () => (
      <>
        ...
        <div className="field-group">
          ...
          <ChannelSelect value={form.music_vc_channel_id} onChange={set("music_vc_channel_id")} allowNone />
        </div>
        <div className="toggle-row">
          <div>
          ...
          <Toggle checked={!!form.music_vc_drag_users} onChange={set("music_vc_drag_users")} />
        </div>
        <div className="field-group">
          <label>Default Volume (1-200)</label>
          <input className="field-input" type="number" min="1" max="200" value={form.music_volume || 100} onChange={(e) => set("music_volume")(parseInt(e.target.value) || 100)} />
        </div>
        <div className="field-group">
          <label>Default Loop Mode</label>
          <select className="field-input" value={form.music_loop_mode || "off"} onChange={(e) => set("music_loop_mode")(e.target.value)}>
        ...
        <div className="toggle-row">
          ...
          <Toggle checked={!!form.music_status_enabled} onChange={set("music_status_enabled")} />
        </div>
      </>
    );
    ...
    const renderLeveling = () => (
      <>
        ...
        <div className="field-group">
          ...
          <select className="field-input" multiple value={(form.xp_blacklist || []).map(String)} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, opt => Number(opt.value)); set("xp_blacklist")(selected); }} style={{ height: "auto", minHeight: "60px" }}>
            {channelOptions.map((c) => <option key={c.id} value={c.id}>#{c.name}</option>)}
          </select>
        </div>
        <div className="field-group">
          <label>XP Blacklisted Roles</label>
          <RoleMultiSelect value={form.xp_role_blacklist || []} onChange={set("xp_role_blacklist")} roleOptions={roleOptions} placeholder="Select XP blacklisted roles..." />
        </div>
        <div className="field-group">
          <label>Level Roles (format: level:role_id)</label>
          <textarea className="field-input" rows="2" value={Object.entries(form.level_roles || {}).map(([lvl, rid]) => `${lvl}:${rid}`).join("\n")} placeholder="10:123456789\n20:987654321" onChange={(e) => { const obj = {}; e.target.value.split("\n").forEach(line => { const [lvl, rid] = line.split(":"); if (lvl && rid) obj[lvl.trim()] = parseInt(rid.trim()); }); set("level_roles")(obj); }} />
        </div>
        <div className="field-group">
          <label>Level-Up Outside Text</label>
          <input className="field-input" value={levelUpConfig.outside_text || "{user}"} placeholder="{user}" onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, outside_text: e.target.value } }))} />
        </div>
        <div className="field-group">
          <label>Level-Up Embed Title</label>
          <input className="field-input" value={levelUpConfig.embed_title || "⬆️ Level Up!"} placeholder="⬆️ Level Up!" onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, embed_title: e.target.value } }))} />
        </div>
        <div className="field-group">
          <label>Level-Up Embed Description</label>
          <textarea className="field-input" rows="2" value={levelUpConfig.embed_description || "🎉 {user} reached **Level {level}**!"} placeholder="🎉 {user} reached **Level {level}**!" onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, embed_description: e.target.value } }))} />
        </div>
        <div className="field-group">
          <label>Level-Up Embed Footer</label>
          <input className="field-input" value={levelUpConfig.embed_footer || ""} placeholder="Footer text" onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, embed_footer: e.target.value } }))} />
        </div>
        <div className="field-group">
          <label>Level-Up Embed Color (hex)</label>
          <input className="field-input" value={levelUpConfig.embed_color || "#FFD700"} placeholder="#FFD700" onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, embed_color: e.target.value } }))} />
        </div>
        <div className="field-group">
          <label>Display Mode</label>
          <select className="field-input" value={levelUpConfig.display_mode || "both"} onChange={(e) => setForm(f => ({ ...f, level_up_config: { ...f.level_up_config, display_mode: e.target.value } }))}>
            <option value="embed">Embed Only</option><option value="plain">Plain Text Only</option><option value="both">Both</option>
          </select>
        </div>
      </>
    );
    ...
    const renderCounting = () => (
      <>
        ...
        <div className="toggle-row">
          ...
          <Toggle checked={!!counting.allow_same_user} onChange={setNested("counting", "allow_same_user")} />
        </div>
        <div className="toggle-row">
          <div>
          ...
          <Toggle checked={!!counting.reset_on_fail} onChange={setNested("counting", "reset_on_fail")} />
        </div>
        <div className="field-group">
          <label>Cooldown (seconds)</label>
          <input className="field-input" type="number" min="0" value={counting.cooldown || 2} onChange={(e) => setNested("counting", "cooldown")(parseInt(e.target.value) || 0)} />
        </div>
        <div className="field-group">
          <label>Save Limit (-1 = unlimited, 0 = disabled)</label>
          <input className="field-input" type="number" min="-1" value={counting.save_limit !== undefined ? counting.save_limit : 3} onChange={(e) => setNested("counting", "save_limit")(parseInt(e.target.value) || 0)} />
        </div>
        <div className="field-group">
          <label>Save Cooldown (seconds)</label>
          <input className="field-input" type="number" min="0" value={counting.save_cooldown || 3} onChange={(e) => setNested("counting", "save_cooldown")(parseInt(e.target.value) || 0)} />
        </div>
        <div className="field-group">
          <label>Milestones (comma separated)</label>
          <input className="field-input" value={(counting.milestones || [100,250,500,1000,2500,5000,10000]).join(", ")} placeholder="100, 250, 500" onChange={(e) => setNested("counting", "milestones")(e.target.value.split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n)))} />
        </div>
        <div style={{ marginTop: "1rem" }}>
          ...
          {["correct","wrong","milestone","high_score","save"].map((key) => (
            <div key={key} className="field-group" style={{ marginBottom: ".5rem" }}>
              ...
              <input className="field-input" value={counting.emojis?.[key] || "✅"} placeholder={key === "correct" ? "✅" : key === "wrong" ? "❌" : ""} onChange={(e) => setForm(f => ({ ...f, counting: { ...f.counting, emojis: { ...f.counting?.emojis, [key]: e.target.value } } }))} />
            </div>
          ))}
        </div>
      </>
    ...
    const renderAntiNuke = () => (
      <>
        ...
        <div className="field-group">
          ...
          <select className="field-input" value={antinuke.punishment || "timeout"} onChange={(e) => setNested("antinuke", "punishment")(e.target.value)}>
            <option value="warn">Warn</option><option value="timeout">Timeout</option><option value="strip_roles">Strip Roles</option><option value="kick">Kick</option><option value="softban">Softban</option><option value="ban">Ban</option>
          </select>
        </div>
        <div className="field-group">
          <label>Duration (minutes, for timeout)</label>
          <input className="field-input" type="number" min="1" value={antinuke.duration || 60} onChange={(e) => setNested("antinuke", "duration")(parseInt(e.target.value) || 60)} />
        </div>
        <div className="field-group">
          <label>Log Channel</label>
        ...
        <div className="field-group">
          ...
          <input className="field-input" value={(antinuke.global_whitelist || []).join(", ")} placeholder="User IDs (comma separated)" onChange={(e) => setNested("antinuke", "global_whitelist")(e.target.value.split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n)))} />
        </div>
        <div className="field-group">
          <label>Global Role Whitelist</label>
          <RoleMultiSelect value={antinuke.global_whitelist_roles || []} onChange={setNested("antinuke", "global_whitelist_roles")} roleOptions={roleOptions} placeholder="Select whitelisted roles..." />
        </div>
        <div className="field-group">
          <label>Forbidden Channels</label>
          <select className="field-input" multiple value={(antinuke.forbidden_channels || []).map(String)} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, opt => Number(opt.value)); setNested("antinuke", "forbidden_channels")(selected); }} style={{ height: "auto", minHeight: "60px" }}>
            {channelOptions.map((c) => <option key={c.id} value={c.id}>#{c.name}</option>)}
          </select>
        </div>
        <div className="field-group">
          <label>Forbidden Categories</label>
          <select className="field-input" multiple value={(antinuke.forbidden_categories || []).map(String)} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, opt => Number(opt.value)); setNested("antinuke", "forbidden_categories")(selected); }} style={{ height: "auto", minHeight: "60px" }}>
    ...
    const renderAutoTranslate = () => (
      <>
        ...
        <div className="field-group">
          ...
          <select className="field-input" value={autotranslate.default_language || "en"} onChange={(e) => setNested("autotranslate", "default_language")(e.target.value)}>
            <option value="en">English</option><option value="es">Spanish</option><option value="fr">French</option><option value="de">German</option><option value="it">Italian</option><option value="pt">Portuguese</option><option value="ru">Russian</option><option value="ja">Japanese</option><option value="ko">Korean</option><option value="zh-cn">Chinese (Simplified)</option>
          </select>
        </div>
        <div className="field-group">
          <label>Ignored Channels</label>
          <select className="field-input" multiple value={(autotranslate.ignored_channels || []).map(String)} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, opt => Number(opt.value)); setNested("autotranslate", "ignored_channels")(selected); }} style={{ height: "auto", minHeight: "60px" }}>
            {channelOptions.map((c) => <option key={c.id} value={c.id}>#{c.name}</option>)}
          </select>
        </div>
        <div className="field-group">
          <label>Ignored Roles</label>
          <RoleMultiSelect value={autotranslate.ignored_roles || []} onChange={setNested("autotranslate", "ignored_roles")} roleOptions={roleOptions} placeholder="Select ignored roles..." />
        </div>
        <div className="field-group">
          <label>Ignored Users (User IDs)</label>
          <input className="field-input" value={(autotranslate.ignored_users || []).join(", ")} placeholder="123456789, 987654321" onChange={(e) => setNested("autotranslate", "ignored_users")(e.target.value.split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n)))} />
        </div>
      </>
    );
  ...
  return (
    <div className="dash-layout" style={{ display: "flex", gap: "1.5rem", alignItems: "flex-start", flexWrap: "wrap" }}>
      ...
      <div className="dash-sidebar" style={{ 
        ...
        overflowY: "auto",
        // Mobile: switch to horizontal scrollbar
