...
function AiModerationSuggest({ guildId }) {
  ...
  const handleAsk = async () => {
    ...
    setSuggestion("");
    try {
      const res = await fetch(`/api/guilds/${guildId}/ai-suggest`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="ai-suggest-box">
      <div className="field-group" style={{ marginBottom: "0.75rem" }}>
        ...
        <textarea
          ...
          rows={4}
          placeholder="e.g. A member has posted 3 spam links in #general in the last minute and is ignoring warnings from other members..."
          value={situation}
          onChange={(e) => setSituation(e.target.value)}
        />
      </div>
      <button className="btn btn-primary" onClick={handleAsk} disabled={loading || !situation.trim()}>
        {loading ? "Thinking..." : "✦ Get AI Suggestion"}
      </button>
      {error && <p style={{ color: "var(--db-err)", marginTop: "0.6rem", fontSize: "0.85rem" }}>{error}</p>}
      {suggestion && <div className="ai-suggest-result">{suggestion}</div>}
    </div>
  );
}
...
function ApplicationsManager({ guildId, apps, onAppsChange, roleOptions, channelOptions }) {
  ...
  const handleToggleApp = async (name, open) => {
    try {
      ...
      if (!res.ok) {
        const error = await res.text();
        throw new Error(error || "Failed to toggle application");
      }
...
export default function SettingsForm({ guildId, initial }) {
  ...
  // Voice channel selector – filters only voice channels from effectiveChannels
  const VoiceChannelSelect = ({ value, onChange, allowNone = true }) => {
    const voiceChannels = effectiveChannels.filter(c =>
  ...
  const navItems = [
    ...
    { icon: "🛡️", label: "Moderation", section: "moderation" },
  ];
  ...
  const renderContent = () => {
    ...
    const renderWelcome = () => (
      <>
        ...
        <div className="field-group"><label>Auto-Roles</label><RoleMultiSelect value={form.autoroles || []} onChange={set("autoroles")} roleOptions={effectiveRoles} placeholder="Select auto-roles..." /></div>
    ...
    const renderAutoMod = () => (
      <>
        ...
        <div className="field-group"><label>Ignored Channels</label><ChannelMultiSelect value={automod.ignored_channels || []} onChange={setNested("automod", "ignored_channels")} channelOptions={effectiveChannels} placeholder="Select ignored channels..." /></div>
        <div className="field-group"><label>Ignored Roles</label><RoleMultiSelect value={automod.ignored_roles || []} onChange={setNested("automod", "ignored_roles")} roleOptions={effectiveRoles} placeholder="Select ignored roles..." /></div>
      </>
    );
    ...
    const renderVerification = () => (
      <>
        ...
        <div className="field-group"><label>Verified Role</label><select className="field-input" value={form.verification_role_id || ""} onChange={(e) => set("verification_role_id")(e.target.value || null)}><option value="">None</option>{effectiveRoles.map((r) => <option key={r.id} value={r.id}>@{r.name}</option>)}</select></div>
    ...
    const renderTickets = () => (
      <>
        ...
        <div className="field-group"><label>Staff Roles (global)</label><RoleMultiSelect value={form.ticket_staff_roles || []} onChange={set("ticket_staff_roles")} roleOptions={effectiveRoles} placeholder="Select staff roles..." /></div>
    ...
    const renderLeveling = () => (
      <>
        ...
        <div className="field-group"><label>XP Blacklisted Channels</label><ChannelMultiSelect value={form.xp_blacklist || []} onChange={set("xp_blacklist")} channelOptions={effectiveChannels} placeholder="Select XP blacklisted channels..." /></div>
        <div className="field-group"><label>XP Blacklisted Roles</label><RoleMultiSelect value={form.xp_role_blacklist || []} onChange={set("xp_role_blacklist")} roleOptions={effectiveRoles} placeholder="Select XP blacklisted roles..." /></div>
    ...
    const renderAntiNuke = () => (
      <>
        ...
        <div className="field-group"><label>Global Role Whitelist</label><RoleMultiSelect value={antinuke.global_whitelist_roles || []} onChange={setNested("antinuke", "global_whitelist_roles")} roleOptions={effectiveRoles} placeholder="Select whitelisted roles..." /></div>
        <div className="field-group"><label>Forbidden Channels</label><ChannelMultiSelect value={antinuke.forbidden_channels || []} onChange={setNested("antinuke", "forbidden_channels")} channelOptions={effectiveChannels} placeholder="Select forbidden channels..." /></div>
        <div className="field-group"><label>Forbidden Categories</label><CategoryMultiSelect value={antinuke.forbidden_categories || []} onChange={setNested("antinuke", "forbidden_categories")} categoryOptions={effectiveChannels.filter(c => c.type === 4)} placeholder="Select forbidden categories..." /></div>
    ...
    const renderAutoTranslate = () => (
      <>
        ...
        <div className="field-group"><label>Ignored Channels</label><ChannelMultiSelect value={autotranslate.ignored_channels || []} onChange={setNested("autotranslate", "ignored_channels")} channelOptions={effectiveChannels} placeholder="Select ignored channels..." /></div>
        <div className="field-group"><label>Ignored Roles</label><RoleMultiSelect value={autotranslate.ignored_roles || []} onChange={setNested("autotranslate", "ignored_roles")} roleOptions={effectiveRoles} placeholder="Select ignored roles..." /></div>
    ...
    const renderModeration = () => (
      <>
        <SectionHeader icon="🛡️" title="Moderation" />
        <div className="field-group">
          <label>Jail Role</label>
          <p className="hint" style={{ marginTop: "-0.2rem", marginBottom: "0.4rem" }}>
            Members hit with <code>//jail</code> / <code>/mod jail</code> get this role in place of their normal roles.
          </p>
          <select className="field-input" value={form.jail_role || ""} onChange={(e) => set("jail_role")(e.target.value || null)}>
            <option value="">None</option>
            {effectiveRoles.map((r) => <option key={r.id} value={r.id}>@{r.name}</option>)}
          </select>
        </div>

        <div style={{ marginTop: "2rem" }}>
          <label style={{ display: "block", marginBottom: "0.5rem", fontWeight: 600, color: "var(--db-text)" }}>
            ✦ AI Suggestions
          </label>
          <p className="hint" style={{ marginTop: 0, marginBottom: "0.75rem" }}>
            Not sure how to handle something? Describe what happened and get a suggested moderation action.
          </p>
          <AiModerationSuggest guildId={guildId} />
        </div>
      </>
    );
  ...
  // ---------- Render ----------
  const guildInfo = initial.guild || {};
  return (
    <>
      {guildInfo.name && (
        <div className="dash-guild-header">
          <div className="dash-guild-header-left">
            {guildInfo.icon ? (
              <img src={guildInfo.icon} alt="" className="dash-guild-icon" />
            ) : (
              <div className="dash-guild-icon dash-guild-icon-fallback">{guildInfo.name.charAt(0).toUpperCase()}</div>
            )}
            <div>
              <h1 className="dash-guild-name">{guildInfo.name}</h1>
              {typeof guildInfo.member_count === "number" && (
                <span className="dash-guild-members">{guildInfo.member_count.toLocaleString()} members</span>
              )}
            </div>
          </div>
          {guildInfo.is_premium && (
            <div className="dash-premium-badge" title="This server has SparkyBot Premium">
              <span className="dash-premium-badge-icon">✦</span> Premium
            </div>
          )}
        </div>
      )}
        ...
        {navItems.map((item) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              transition: "background-color 0.2s var(--ease-smooth, ease), color 0.2s var(--ease-smooth, ease)",
              whiteSpace: "nowrap",
              overflow: "hidden",
