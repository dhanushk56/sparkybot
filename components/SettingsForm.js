...
function ApplicationsManager({ guildId, apps, onAppsChange, roleOptions, channelOptions, userId }) {
  ...
  // ----- Delete Application (FIXED: includes user_id) -----
  const handleDeleteApp = async (name) => {
    if (!confirm(`Delete application "${name}"? This cannot be undone.`)) return;
    try {
      const encodedName = encodeURIComponent(name);
      // The API expects a user_id query parameter for audit logging
      const res = await fetch(`/api/guilds/${guildId}/applications/${encodedName}?user_id=${userId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const error = await res.text();
...
// ---------- Main SettingsForm Component (accepts userId) ----------
export default function SettingsForm({ guildId, initial, userId }) {
